...
{
    XLabels_t *xlp = (XLabels_t *) gv_alloc(sizeof(XLabels_t));
...
xlintersections(XLabels_t * xlp, object_t * objp, object_t * intrsx[XLNBR])
{
	...
	double a, ra;
	object_t *cp = (object_t *)ilp->leaf->data;
...
static int xlhdxload(XLabels_t * xlp)
{
	...
	hp = (HDict_t *)gv_alloc(sizeof(HDict_t));
...
{
    for (HDict_t *op = (HDict_t *) dtfirst(xlp->hdx); op; op = (HDict_t *) dtnext(xlp->hdx, op)) {
