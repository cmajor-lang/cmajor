...
{
    int i;
    rawgraph* g = (rawgraph*) gv_alloc(sizeof(rawgraph));
    g->nvs = n;
    g->vertices = (vertex*)gv_calloc(n, sizeof(vertex));
    for(i=0;i<n;i++) {
...
mkStack (int i)
{
    stack* sp = (stack*) gv_alloc(sizeof(stack));
    sp->vals = (int*) gv_calloc(i, sizeof(int));
    sp->top = -1;
    return sp;
}
