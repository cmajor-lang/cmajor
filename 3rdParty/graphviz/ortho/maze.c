...
{
    snodeitem* n = (snodeitem*)dtmatch (cdt, &p);
...
mkMazeGraph (maze* mp, boxf bb)
{
    ...
    snodeitem* ditems = (snodeitem*) gv_calloc(bound, sizeof(snodeitem));
    snode** sides;
     ...
     */
    sides = (snode**) gv_calloc(4 * mp->ncells, sizeof(snode*));
     ...
     */
    maxdeg = 0;
    sides = (snode**) gv_calloc(g->nnodes, sizeof(snode*));
    nsides = 0;
	...
	cp->sides = sides+nsides;
        pt = cp->bb.LL;
	np =(snodeitem*) dtmatch (hdict, &pt);
	for (; np && np->p.x < cp->bb.UR.x; np = (snodeitem*) dtnext (hdict, np)) {
	    cp->sides[cp->nsides++] = np->np;
	    np->np->cells[1] = cp;
	}
	np =(snodeitem*) dtmatch (vdict, &pt);
	for (; np && np->p.y < cp->bb.UR.y; np =(snodeitem*) dtnext (vdict, np)) {
	    cp->sides[cp->nsides++] = np->np;
	    np->np->cells[1] = cp;
	}
	pt.y = cp->bb.UR.y;
...
maze *mkMaze(graph_t *g) {
    node_t* n;
    maze* mp =(maze*) gv_alloc(sizeof(maze));
    boxf* rects;
    cell* cp;
    double w2, h2;
    boxf bb, BB;
    ...
    cp = mp->gcells = (cell*) gv_calloc(mp->ngcells, sizeof(cell));
...
#endif
    mp->cells = (cell*)gv_calloc(nrect, sizeof(cell));
