...
convertSPtoRoute (sgraph* g, snode* fst, snode* lst)
{
    ...
    rte.n = 0;
    assert(sz >= 2);
    rte.segs = (segment*)gv_calloc(sz - 2, sizeof(segment));  /* at most sz-2 segments */
    ...
    rte.segs = (segment*) realloc (rte.segs, rte.n*sizeof(segment));
...
{
    chanItem* subd = (chanItem*) dtmatch (chdict, &j);

    if (!subd) {
	subd = (chanItem*) gv_alloc(sizeof(chanItem));
	subd->v = j;
...
extractHChans (maze* mp)
{
	...
	chp = (channel*) gv_alloc(sizeof(channel));
	chp->cp = cp;
	chp->p.p1 = cp->bb.LL.x;
...
{
  channel* cp;
  chanItem* chani = (chanItem*) dtmatch (chans, &seg->comm_coord);
  assert (chani);
  cp = (channel*) dtmatch (chani->chans, &seg->p);
  assert (cp);
  return cp;
}
...
static int
eqEndSeg (bend_ S1l2, bend_ S2l2, bend_ T1, bend_ T2)
{
...
static int
overlapSeg (segment* S1, segment* S2, bend_ T1, bend_ T2)
{
	if(S1->p.p2<S2->p.p2) {
...
static int
ellSeg (bend_ S1l1, bend_ S1l2, bend_ T)
{
    if (S1l1 == T) {
	if (S1l2== T) return -1;
	else return 0;
...
static int
segCmp (segment* S1, segment* S2, bend_ T1, bend_ T2)
{
	/* no overlap */
 ...
 */
static int
decide_point(pair_ *ret, segment* si, segment* sj, int dir1, int dir2)
{
    int prec, ans = 0, temp;
    segment* np1 = NULL;
    segment* np2 = NULL;
...
attachOrthoEdges(maze* mp, size_t n_edges, route* route_list, splineInfo *sinfo, epair_t es[], int doLbls)
{
	...
	if (npts > splsz) {
		free (ispline);
		ispline = (pointf*)gv_calloc(npts, sizeof(pointf));
		splsz = npts;
	}
...
static int edgecmp2(epair_t* e0, epair_t* e1)
{
    return e0->d - e1->d;
}

static bool spline_merge2(node_t * n)
{
    (void)n;
    return false;
}

static bool swap_ends_p2(edge_t * e)
{
    (void)e;
    return false;
}

static splineInfo sinfo2 = { swap_ends_p2, spline_merge2, true, true };
...
orthoEdges (Agraph_t* g, int doLbls)
{
    ...
    snode* sn;
    snode* dn;
    epair_t* es = (epair_t*)gv_calloc(agnedges(g), sizeof(epair_t));
    cell* start;
    cell* dest;
    PointSet* ps = NULL;
    textlabel_t* lbl;
    ...
    route_list = (route*) gv_calloc(n_edges, sizeof(route));

    qsort(es, n_edges, sizeof(epair_t), (qsort_cmpf) edgecmp2);
...
#endif
    attachOrthoEdges(mp, n_edges, route_list, &sinfo2, es, doLbls);
