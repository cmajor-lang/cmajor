...
struct Initialiser9
{
    Initialiser9()
    {
		sinfo.swapEnds = swap_ends_p;
        sinfo.splineMerge = spline_merge;
    }
};

static Initialiser9 initialiser9;
...
/* swap_bezier:
 */
static void swap_bezier(bezier * old, bezier * new_)
{
    pointf *list;
    pointf *lp;
    pointf *olp;
    ...
    sz = old->size;
    list = (pointf*) gv_calloc(sz, sizeof(pointf));
    lp = list;
    olp = old->list + (sz - 1);
	...
	*lp++ = *olp--;
    ...
    new_->list = list;
    new_->size = sz;
    new_->sflag = old->eflag;
    new_->eflag = old->sflag;
    new_->sp = old->ep;
    new_->ep = old->sp;
}
...
static void swap_spline(splines * s)
{
    ...
    sz = s->size;
    list = (bezier*) gv_calloc(sz, sizeof(bezier));
    lp = list;
    olp = s->list + (sz - 1);
...
static void _dot_splines(graph_t * g, int normalize)
{
    ...
    edges = (edge_t**)gv_calloc(CHUNK, sizeof(edge_t*));
    ...
    P.boxes = (boxf*) gv_calloc(n_nodes + 20 * 2 * NSUB, sizeof(boxf));
    sd.Rank_box = (boxf*) gv_calloc(i, sizeof(boxf));
	...
	if (et == EDGETYPE_CURVED) {
		...
		edgelist = (edge_t**)gv_calloc(cnt, sizeof(edge_t*));
...
makeSimpleFlatLabels (node_t* tn, node_t* hn, edge_t** edges, int ind, int cnt, int et, int n_lbls)
{
    ...
    edge_t** earray = (edge_t**)gv_calloc(cnt, sizeof(edge_t*));
...
make_flat_labeled_edge(graph_t* g, spline_info_t* sp, path* P, edge_t* e, int et)
{
	...
	boxf boxes[] = {
	  {{tend.boxes[tend.boxn - 1].LL.x,
	    tend.boxes[tend.boxn - 1].UR.y, },
	    lb.LL, },
	  {{tend.boxes[tend.boxn - 1].LL.x, lb.LL.y, },
	    {hend.boxes[hend.boxn - 1].UR.x, lb.UR.y, }, },
	  { {lb.UR.x,hend.boxes[hend.boxn - 1].UR.y, },
	    { hend.boxes[hend.boxn - 1].UR.x, lb.LL.y, }, },
	};
...
make_regular_edge(graph_t* g, spline_info_t* sp, path * P, edge_t ** edges, int ind, int cnt, int et)
{
    ...
    if (!pointfs) {
	pointfs = (pointf*)gv_calloc(NUMPTS, sizeof(pointf));
   	pointfs2 = (pointf*)gv_calloc(NUMPTS, sizeof(pointf));
	numpts = NUMPTS;
	numpts2 = NUMPTS;
    }
    sl = 0;
