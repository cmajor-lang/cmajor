...
static node_t* makeXnode (graph_t* G, char* name)
{
    ...
    if (Last_node_) {
	ND_prev(n) = Last_node_;
	ND_next(Last_node_) = n;
    } else {
	ND_prev(n) = NULL;
	GD_nlist(G) = n;
    }
...
{ int *sz = (int*)arg; (void)g; agbindrec(graph,"level graph rec",sz[0],true); }
static void my_init_node(Agraph_t *g, Agobj_t *node, void *arg)
{ int *sz = (int*) arg; (void)g; agbindrec(node,"level node rec",sz[1],true); }
static void my_init_edge(Agraph_t *g, Agobj_t *edge, void *arg)
{ int *sz = (int*) arg; (void)g; agbindrec(edge,"level edge rec",sz[2],true); }
