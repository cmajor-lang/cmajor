	...
	int ssize, pack_info * pinfo, point center)
{
	...
	/* backup the alg data */
	void **alg = (void**) gv_calloc(agnnodes(g), sizeof(void*));
...
arrayRects (int ng, boxf* gs, pack_info* pinfo)
{
    ...
    ainfo *info;
    double v, wd, ht;
    point *places = (point*) gv_calloc(ng, sizeof(point));
    boxf bb;
    int sz, rowMajor;
    ...
    double *widths = (double*)gv_calloc(nc + 1, sizeof(double));
    double *heights = (double*)gv_calloc(nr + 1, sizeof(double));

    ainfo *ip = info = (ainfo*) gv_calloc(ng, sizeof(ainfo));
    for (i = 0; i < ng; i++, ip++) {
	bb = gs[i];
    ...
    ainfo **sinfo = (ainfo **)gv_calloc(ng, sizeof(ainfo*));
    for (i = 0; i < ng; i++) {
	sinfo[i] = info + i;
    }
...
polyRects(int ng, boxf* gs, pack_info * pinfo)
{
    ...
    /* sort */
    ginfo **sinfo =(ginfo**) gv_calloc(ng, sizeof(ginfo*));
    for (i = 0; i < ng; i++) {
	sinfo[i] = info + i;
    }
...
polyGraphs(int ng, Agraph_t ** gs, Agraph_t * root, pack_info * pinfo)
{
    ...
    /* calculate grid size */
    boxf *bbs = (boxf*) gv_calloc(ng, sizeof(boxf));
    for (i = 0; i < ng; i++)
	bbs[i] = GD_bb(gs[i]);
	...
	center.x = center.y = 0;
    info = (ginfo*) gv_calloc(ng, sizeof(ginfo));
    for (i = 0; i < ng; i++) {
	Agraph_t *g = gs[i];
	info[i].index = i;
