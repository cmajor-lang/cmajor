...
static Agsym_t *agnewsym(Agraph_t * g, const char *name, const char *value,
                         int id, int kind) {
    Agsym_t *sym;
    sym = (Agsym_t*) agalloc(g, sizeof(Agsym_t));
...
static Agrec_t *agmakeattrs(Agraph_t * context, void *obj)
{
    ...
    rec = (Agattr_t*) agbindrec(obj, AgDataRecName, sizeof(Agattr_t), false);
    datadict = agdictof(context, AGTYPE(obj));
    assert(datadict);
    if (rec->dict == NULL) {
	rec->dict = agdictof(agroot(context), AGTYPE(obj));
	/* don't malloc(0) */
	sz = topdictsize((Agobj_t*) obj);
	if (sz < MINATTR)
	    sz = MINATTR;
	rec->str = (char**) agalloc(agraphof(obj), (size_t) sz * sizeof(char *));
	/* doesn't call agxset() so no obj-modified callbacks occur */
	for (sym = (Agsym_t*) dtfirst(datadict); sym; sym = (Agsym_t*) dtnext(datadict, sym))
...
char *agxget(void *obj, Agsym_t * sym)
{
    ...
    data = agattrrec(obj);
    assert(sym->id >= 0 && sym->id < topdictsize((Agobj_t*) obj));
    rv = data->str[sym->id];
    return rv;
}
...
int agsafeset(void *obj, const char *name, const char *value, const char *def) {
    Agsym_t *a;
