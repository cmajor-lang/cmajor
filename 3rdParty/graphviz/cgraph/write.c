...
static int write_dicts(Agraph_t * g, iochan_t * ofile, int top)
{
	...
	CHKRV(write_dict(g, ofile, (char*) "graph", def->dict.g, top));
	CHKRV(write_dict(g, ofile, (char*) "node", def->dict.n, top));
	CHKRV(write_dict(g, ofile, (char*) "edge", def->dict.e, top));
    }
    return 0;
}
...
static int write_hdr(Agraph_t * g, iochan_t * ofile, int top)
{
	...
	CHKRV(ioput(g, ofile, (char*) "graph "));
    }
    if (hasName)
...
static bool not_default_attrs(Agraph_t * g, Agnode_t * n)
{
    ...
    (void)g;
    if ((data = agattrrec(n))) {
	for (sym = (Agsym_t*)dtfirst(data->dict); sym; sym = (Agsym_t*)dtnext(data->dict, sym)) {
				  ...
				  Dict_t * defdict)
{
	...
	CHKRV(rv = write_edge_name((Agedge_t*)obj, ofile, FALSE));
	if (rv)
	    cnt++;
    }
    data = agattrrec(obj);
...
static int write_edge(Agedge_t * e, iochan_t * ofile, Dict_t * d)
{
    ...
    CHKRV(ioput(g, ofile, (char*) ((agisdirected(agraphof(t)) ? " -> " : " -- "))));
