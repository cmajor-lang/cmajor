...
obj_state_t* push_obj_state(GVJ_t *job)
{
    ...
    obj = (obj_state_t*)zmalloc(sizeof(obj_state_t));
...
wedgedEllipse (GVJ_t* job, pointf * pf, char* clrs)
{
	...
	if (s->t == 0) continue;
	gvrender_set_fillcolor (job, (char*) (s->color?s->color:DEFAULT_COLOR));
...
static void map_bspline_poly(pointf **pbs_p, int **pbs_n, int *pbs_poly_n, int n, pointf* p1, pointf* p2)
{
    ...
    (*pbs_poly_n)++;
    *pbs_n = (int*)grealloc(*pbs_n, (*pbs_poly_n) * sizeof(int));
    (*pbs_n)[i] = 2*n;
    *pbs_p = (pointf*)grealloc(*pbs_p, (nump + 2*n) * sizeof(pointf));
...
static bool write_edge_test_(Agraph_t * g, Agedge_t * e)
{
    Agraph_t *sg;
    int c;
...
static void emit_attachment(GVJ_t * job, textlabel_t * lp, splines * spl)
{
    ...
    sz = lp->dimen;
    AF[0] = {lp->pos.x + sz.x / 2., lp->pos.y - sz.y / 2.};
    AF[1] = {AF[0].x - sz.x, AF[0].y};
...
#define SEP 2.0
	...
	/* if more than one color - then generate parallel beziers, one per color */
	    ...
	    offspl.list = (bezier *) malloc(sizeof(bezier) * offspl.size);
	    tmpspl.list = (bezier *) malloc(sizeof(bezier) * tmpspl.size);
	    numc2 = (2 + numc) / 2.0;
	    for (i = 0; i < offspl.size; i++) {
		bz = ED_spl(e)->list[i];
		tmpspl.list[i].size = offspl.list[i].size = bz.size;
		offlist = offspl.list[i].list = (pointf*) malloc(sizeof(pointf) * bz.size);
		tmplist = tmpspl.list[i].list = (pointf*) malloc(sizeof(pointf) * bz.size);
		pf3 = bz.list[0];
...
static void
emit_edge_label(GVJ_t* job, textlabel_t* lbl, emit_state_t lkind, int explicit_,
    char* url, char* tooltip, char* target, char *id, splines* spl)
{
    ...
    if ((url || explicit_) && !(flags & EMIT_CLUSTERS_LAST)) {
	map_label(job, lbl);
...
static void emit_edge(GVJ_t * job, edge_t * e)
{
	...
	s = (char*)malloc(strlen(agnameof(agtail(e))) + 2 + strlen(agnameof(aghead(e))) + 1);
...
boxf xdotBB (Agraph_t* g)
{
	...
	case xd_text :
	    ...
	    tf.size = fontsize;
	    tf.flags = fontflags;
            op->span->font = (textfont_t*) dtinsert(gvc->textfont_dt, &tf);
...
static void init_job_viewport(GVJ_t * job, graph_t * g)
{
    ...
    if ((str = agget(g, "viewport"))) {
        nodename = (char*)malloc(strlen(str)+1);
...
struct Initialiser6
{
    ...
    {
		stringdict.link = -1;
		stringdict.freef = (Dtfree_f)free_string_entry;
    }
};

static Initialiser6 initialiser6;
