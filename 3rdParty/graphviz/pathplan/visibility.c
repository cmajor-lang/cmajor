...
#else
#define INTERSECT(a,b,c,d) intersect((a),(b),(c),(d))
#endif
...
static array2 allocArray(int V, int extra)
{
    ...
    assert(V >= 0);
    array2 arr = (array2) gv_calloc(V + extra, sizeof(COORD*));
    COORD *p = (COORD*) gv_calloc((size_t)V * (size_t)V, sizeof(COORD));
    for (i = 0; i < V; i++) {
	arr[i] = p;
	p += V;
    }
 ...
 */
static COORD dist3(Ppoint_t a, Ppoint_t b)
{
