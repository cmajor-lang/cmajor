...
static void xdot_point_(agxbuf *xb, pointf p)
{
    char buf[BUFSIZ];
...
static void xdot_points(GVJ_t *job, char c, pointf * A, int n)
{
    ...
    for (i = 0; i < n; i++)
        xdot_point_(xbufs[emit_state], A[i]);
}
 ...
 */
static void
xdot_begin_graph (graph_t *g, int s_arrows, int e_arrows, format_type2 id)
{
    int i;
    unsigned short us;
    char* s;

    xd = (xdot_state_t *)gv_alloc(sizeof(xdot_state_t));
...
static void dot_begin_graph(GVJ_t *job)
{
	...
	case FORMAT_XDOT14:
	    ...
	    xdot_begin_graph(g, s_arrows, e_arrows, (format_type2) job->render.id);
	    break;
	default:
	    UNREACHABLE();
    }
...
static void xdot_textspan(GVJ_t * job, pointf p, textspan_t * span)
{
    ...
    xdot_point_(xbufs[emit_state], p);
...
static void xdot_color_stop_ (agxbuf* xb, float v, gvcolor_t* clr)
{
    char buf[BUFSIZ];
...
static void xdot_gradient_fillcolor (GVJ_t* job, int filled, pointf* A, int n)
{
	...
	xdot_color_stop_ (&xb, obj->gradient_frac, &obj->fillcolor);
	xdot_color_stop_ (&xb, obj->gradient_frac, &obj->stopcolor);
    }
    else {
	xdot_color_stop_ (&xb, 0, &obj->fillcolor);
	xdot_color_stop_ (&xb, 1, &obj->stopcolor);
    }
    agxbpop(&xb);
    if (filled == GRADIENT)
	agxbputc(&xb, ']');
...
static void xdot_ellipse(GVJ_t * job, pointf * A, int filled)
{
	...
	if ((filled == GRADIENT) || (filled == RGRADIENT)) {
	   xdot_gradient_fillcolor (job, filled, A, 2);
        ...
        agxbput(xbufs[emit_state], "e ");
    xdot_point_(xbufs[emit_state], A[0]);
...
static void xdot_polyline_(GVJ_t * job, pointf * A, int n)
{
    xdot_style (job);
    xdot_pencolor (job);
...
void core_loadimage_xdot(GVJ_t * job, usershape_t *us, boxf b, bool filled)
{
    ...
    xdot_point_(xbufs[emit_state], b.LL);
