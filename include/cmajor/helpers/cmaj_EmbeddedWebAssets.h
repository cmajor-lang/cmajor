...
private:
    ...
    static constexpr const char* cmaj_midi_helpers_js =
        R"(//  //
//  //     ,ad888ba,                                88
//  //    d8"'    "8b
...
export function getMIDIDescription (message)
{
    ...
    if (isController (message))            return "Controller:" + channelText + ": " + getControllerName (getControllerNumber()) + " = " + getControllerValue (message);
...
R"TEXT(
    if (isSongPositionPointer (message))   return "Song Position: " + getSongPositionPointerValue();
...
export default class LevelMeter extends HTMLElement
{
    ...
    setNumChans (numChans)
    {
        if (this.numChans != numChans)
        {
            ...
            this.currentLevels = Array (numChans).fill (0.0);
            this.refreshContent();
        }
    }

    setLevels (minLevels, maxLevels)
    {
        if (minLevels && maxLevels
             && minLevels.constructor === Array
             && maxLevels.constructor === Array)
        {
            const numChans = Math.min (minLevels.length, maxLevels.length);
            this.setNumChans (numChans);

            for (let i = 0; i < numChans; ++i)
            {
                const maxAbs = Math.max (Math.abs (minLevels[i]),
                                         Math.abs (maxLevels[i]));
                this.setChannelLevel (i, maxAbs);
            }
        }
    }
    ...
    setChannelLevel (channel, newLevel)
    {
        ...
        const last = this.currentLevels[channel];

        if (newLevel > last)
            newLevel = Math.min (1.0, newLevel)
        else if (last > 0.001)
            newLevel = last * decayFactor;
        else
            newLevel = 0;

        newLevel = Math.floor (newLevel * 100.0) / 100.0;

        if (this.currentLevels[channel] != newLevel)
        {)"
R"(
            this.currentLevels[channel] = newLevel;
            const elem = this.root.querySelector(`[data-chan="${channel}"]`);
            elem.style.height = `${newLevel * 100}%`;
        }

        if (! this.lastHeight || this.clientHeight != this.lastHeight)
        {
    ...
    refreshContent()
    {
        ...
        for (let chan = 0; chan < this.numChans; ++chan)
            bars += `<div class="meter-bar" data-chan="${chan}">${chan}</div>`;
        ...
        return `
            * {
            ...
            .meter-bar {
                ...
                background-size: 100% ${this.lastHeight}px;
...
customElements.define ("cmaj-level-meter", LevelMeter);
    ...
    static constexpr const char* generic_patch_view_js =
        R"(//  //
//  //     ,ad888ba,                                88
//  //    d8"'    "8b
//  //   d8            88,dPba,,adPba,   ,adPPYba,  88      The Cmajor Language
...
class PatchView extends HTMLElement
{
    ...
    connectedCallback()
    {
        ...
        font.href = this.patchConnection.getResourceAddress ("/cmaj_api/assets/ibmplexmono/v12/ibmplexmono.css");
        font.rel = "stylesheet"
...
.logo {
    ...
    mask: url(${this.patchConnection.getResourceAddress ("/cmaj_api/assets/sound-stacks-logo.svg")});
    mask-repeat: no-repeat;
    -webkit-mask: url(${this.patchConnection.getResourceAddress ("/cmaj_api/assets/sound-stacks-logo.svg")});
...
if (! window.customElements.get ("cmaj-generic-patch-view"))
    ...
    static constexpr const char* cmaj_piano_keyboard_js =
        R"(//  //
//  //     ,ad888ba,                                88
//  //    d8"'    "8b
//  //   d8            88,dPba,,adPba,   ,adPPYba,  88      The Cmajor Language
...
import * as midi from "./cmaj_midi_helpers.js"
    ...
    getNoteElements()
    {
        ...
        for (let i = 0; i < config.numNotes; ++i)
        {
            ...
            if (midi.isNatural (note))
                ...
                accidentals += `<div class="accidental-note note" id="note${note}" style="left: ${x + accOffset}px"></div>`;
