...
struct PatchPlayerServer
{
    ...
    {
        httpServer = std::make_shared<cmaj::server::HTTPServer> (address, port, [this]
        {
            return std::make_unique<ClientRequestHandler> (*this);
        });
    ...
    choc::value::Value getAudioDeviceProperties()
    {
        if (audioPlayer != nullptr)
        {
            ...
            return choc::json::create (
                     ...
                     "sampleRates", choc::value::createArray (availableDevices.sampleRates),
                     "blockSizes", choc::value::createArray (availableDevices.blockSizes),
                     "audioAPI", o.audioAPI,
    ...
    //==============================================================================
    struct ClientRequestHandler  : public cmaj::server::HTTPServer::ClientInstance
    {
        ...
        std::variant<std::string, std::filesystem::path> getHTTPContent (std::string_view path) override
        {
            if (path == "/index.html")
                return createRedirectToNewSessionPage();
            ...
            if (auto sessionIndex = path.find ("/session_"); sessionIndex != std::string_view::npos)
            {
                ...
                if (auto slash = sessionID.find ("/"); slash != std::string_view::npos)
                {
                    ...
                    if (currentSession != nullptr)
                    {
                        auto result = currentSession->serveHTTPRequest (path);
                        auto p = std::get_if<std::filesystem::path> (&result);

                        if (p == nullptr || *p != std::filesystem::path())
                            return result;
                    }
                }
            }
    ...
    struct Session
    {
        ...
        std::variant<std::string, std::filesystem::path> serveHTTPRequest (std::string_view path)
        {
            if (path == "/cmaj-patch-chooser.html")  return createPatchChooserPage();
            if (path == "/cmaj-patch-runner.html")   return createPatchRunnerPage();
    ...
    void dumpActiveSessionStats()
    {
        ...
        for (auto& s : activeSessions)
        {
            table << s.first << s.second->getSessionAge();
            table.newRow();
        }
...
private:
    ...
    std::shared_ptr<cmaj::server::HTTPServer> httpServer;
